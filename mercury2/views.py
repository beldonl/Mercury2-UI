...
def gs_overview(request):
	...
	if request.method == 'POST':
		...
		form = addPipeLine()
	return render_to_response('mercury2/gs_overview.html',{'user': request.user, 'form': form, 'substations': Substation.objects.values(), 'pipelines': Pipeline.objects.values(), 'Devices': Device.objects.values(), }, context_instance=RequestContext(request))
